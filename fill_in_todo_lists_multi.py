...
if __name__ == "__main__":
	...
	cursor = conn.cursor()
	counter =0
	for filename in range(1,14):
		filename = '/Volumes/Byeeee/openlibrary/owis'+str(filename)+'.results'
		...
		with open(filename,'r') as f:
			for line in f:
				...
				try:
					...
					cursor.execute('UPDATE raw_results set classify_multi = ? WHERE isbn = ?',(line['results'],line['id']))
